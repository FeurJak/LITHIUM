mod fm;
pub use fm::{File, FileId, FileMap, PathString};
...
pub const FILE_EXTENSION: &str = "circom";
#[derive(Clone)]
...
impl FileManager {
    ...
    pub fn add_file_with_source_canonical_path(
        ...
        let file_id = self.fm.add_file(file_name_path_buf.clone().into(), source);
    ...
    pub fn fetch_file(&self, file_id: FileId) -> Option<&str> {
        ...
        self.fm.get_file(file_id).map(|file| file.source())
    }
    ...
    pub fn name_to_id(&self, file_name: PathBuf) -> Option<FileId> {
        self.fm.get_file_id(&PathString::from_path(file_name))
    }

    /// Find a file by its path suffix, e.g. "src/main.circom" is a suffix of
    /// "some_dir/package_name/src/main.circom"
...
mod tests {
    ...
    fn path_resolve_file_module_other_ext() {
        ...
        let file_id = add_file(&mut fm, &dir.join("foo.circom"));

        assert!(fm.path(file_id).unwrap().ends_with("foo.circom"));
    }

    /// Tests that two identical files that have different paths are treated as the same file
    /// e.g. if we start in the dir ./src and have a file ../../foo.circom
    /// that should be treated as the same file as ../ starting in ./
    /// they should both resolve to ../foo.circom
    #[test]
    fn path_resolve_modules_with_different_paths_as_same_file() {
        let dir = PathBuf::new();
        let mut fm = FileManager::new(&dir);

        // Create a lib.circom file at the root and add it to the file manager.
        let file_id = add_file(&mut fm, &dir.join("lib.circom"));
        ...
        let second_file_id = add_file(
            &mut fm,
            PathBuf::from(sub_sub_dir.as_path()).join("./../../lib.circom").as_path(),
        );
